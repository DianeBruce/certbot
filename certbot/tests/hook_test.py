...
class PostHookTest(HookTest):
    ...
    def test_certonly_and_run(self):
        self.config.post_hook = "foo"
    ...
    def test_renew(self):
        ...
        for _ in range(2):
            # test that 2nd run has no additional effect
            self.assertFalse(self._call_with_mock_execute(self.config).called)
            self.assertEqual(self._get_eventually(), [self.config.post_hook])

    def test_no_hook(self):
        self.config.post_hook = None
        for verb in ("certonly", "run", "renew",):
            self.config.verb = verb
            self.assertFalse(self._call_with_mock_execute(self.config).called)
            self.assertFalse(self._get_eventually())
...
class RunRenewPostHooksTest(HookTest):
    """Tests for certbot.hooks.run_renew_post_hooks."""

    @classmethod
    def _call(cls, *args, **kwargs):
        from certbot.hooks import run_renew_post_hooks
        return run_renew_post_hooks(*args, **kwargs)

    def _call_with_mock_execute_and_eventually(self, *args, **kwargs):
        """Call run_renew_post_hooks but mock out execute and eventually

        certbot.hooks.post_hook.eventually is replaced with
        self.eventually. The mock execute object is returned rather than
        the return value of run_renew_post_hooks.
    ...
    def test_it(self):
        unique_eventually = ["bar", "baz"]
        self.eventually = unique_eventually + [self.dir_hook]

        mock_execute = self._call_with_mock_execute_and_eventually(self.config)
        executed_hooks = [call[0][0] for call in mock_execute.call_args_list]
        self.assertEqual(executed_hooks, [self.dir_hook] + unique_eventually)
...
class ListHooksTest(util.TempDirTestCase):
    ...
    def test_nonexistent(self):
        self.assertFalse(self._call(os.path.join(self.tempdir, "foo")))
